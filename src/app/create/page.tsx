...
// Platform fee tiers (fixed, not user-configurable).
// Trades below 0.15 BTC pay 1 %; trades at or above 0.15 BTC pay 0.5 %.
const TIER_THRESHOLD_SATS = 15_000_000n; // 0.15 BTC
const FEE_BPS_BELOW = 100; // 1 %
const FEE_BPS_ABOVE = 50;  // 0.5 %

function getPlatformFeeBps(btcSats: bigint): number {
  return btcSats < TIER_THRESHOLD_SATS ? FEE_BPS_BELOW : FEE_BPS_ABOVE;
}
...
function CreateOfferPage() {
  ...
  const currentFeeBps = getPlatformFeeBps(btcSatsRaw);
  const feeDustError = (() => {
    if (btcSatsRaw === 0n) return null;
    if (btcSatsRaw < DUST_THRESHOLD) return `Price below dust threshold (min ${DUST_THRESHOLD} sats).`;
    const feeSats = btcSatsRaw * BigInt(currentFeeBps) / 10_000n;
    if (feeSats < DUST_THRESHOLD) {
      return `Platform fee (${feeSats} sats) below dust threshold. Increase price to at least ${DUST_THRESHOLD * 10_000n / BigInt(currentFeeBps)} sats.`;
    }
    return null;
  })();

  // ── Shared fees calculation ──────────────────────────────────────────────
  // When enabled, the listing price is lowered so seller and buyer split the
  // fee roughly 50/50.  Formula: adjustedSats = desiredSats × (1 − rate/2)
  // The fee is then recalculated on the adjusted price.
  const sharedFeesInfo = (() => {
    if (!sharedFees || btcSatsRaw === 0n) return null;
    const rate = getPlatformFeeBps(btcSatsRaw);
    // adjustedSats = desiredSats * (10_000 - rate/2) / 10_000
    const halfRate = BigInt(Math.floor(rate / 2));
    const adjustedSats = btcSatsRaw * (10_000n - halfRate) / 10_000n;
    const adjustedFeeBps = getPlatformFeeBps(adjustedSats);
    const adjustedFee = adjustedSats * BigInt(adjustedFeeBps) / 10_000n;
  ...
  // The actual sats used for the contract call — adjusted when shared fees is on.
  ...
  // ── Auto-redirect when offer confirmed (also save timestamp + hidden flag) ─
  useEffect(() => {
  ...
  }, [flow.state.phase, flow.state.offerId, router, hiddenListing, address]);
  ...
  const validate = (): string | null => {
    ...
    if (allowedTaker) {
      ...
      if (address && allowedTaker.trim().toLowerCase() === address.trim().toLowerCase()) {
        return 'Private buyer cannot be your own wallet address';
      }
    }
    return null;
  };
    ...
    <>
      ...
      <div className="max-w-xl mx-auto space-y-6">
        ...
        <form onSubmit={(e) => e.preventDefault()} className="card space-y-5">
          ...
          {/* Platform fee — tiered, not user-configurable */}
          <div>
            <div className={`flex items-center justify-between rounded-lg bg-surface px-4 py-3 border ${feeDustError ? 'border-red-700' : 'border-surface-border'}`}>
              ...
              <span className="text-sm font-semibold text-white">
                {currentFeeBps === FEE_BPS_BELOW ? '1%' : '0.5%'}
                {btcSatsRaw > 0n && (
                  <span className="text-xs text-slate-500 font-normal ml-2">
                    = {formatBtcFromSats(BigInt(Math.ceil(Number(btcSatsRaw) * currentFeeBps / 10_000)))}
                  </span>
                )}
              </span>
            </div>
          ...
          <div className={`rounded-xl border p-4 transition-colors duration-200 ${
            ...
            {/* Hidden from Marketplace — always visible, disabled until buyer is set */}
            {(() => {
              const hasBuyer = !!(allowedTaker || privateBuyerLocked);
              const hint = !hasBuyer ? 'enter buyer wallet first' : null;
              return (
                <div className="mt-4 pt-4 border-t border-sky-800/30">
                  <label className={`flex items-center gap-2 mb-2${hasBuyer ? ' cursor-pointer' : ''}`}>
                    <input
                      type="checkbox"
                      checked={hiddenListing}
                      onChange={(e) => hasBuyer && setHiddenListing(e.target.checked)}
                      disabled={!hasBuyer}
                      className={`w-4 h-4 rounded accent-emerald-500${!hasBuyer ? ' opacity-40' : ''}`}
                    />
                    <span className={`text-sm font-semibold${!hasBuyer ? ' text-slate-500' : ' text-slate-200'}`}>
                      Hidden
                    </span>
                    {hint ? (
                      <span className="text-[10px] text-slate-300 font-normal">{hint}</span>
                    ) : (
                      <span className="text-[10px] text-slate-500 font-normal">hide from marketplace</span>
                    )}
                  </label>
                  <p className="text-xs text-slate-500">
                    This listing won&apos;t appear in the public marketplace. Only you and the buyer can see it. Anyone with the direct link can still view the details.
                  </p>

                  {hiddenListing && (
                    <div className="mt-2 rounded-lg border border-sky-800/30 bg-sky-900/10 px-3 py-2">
                      <p className="text-[11px] text-sky-300/80 leading-relaxed">
                        Your listing will be invisible in the marketplace. Share the direct link with your buyer to complete the trade.
                      </p>
                    </div>
                  )}
                </div>
