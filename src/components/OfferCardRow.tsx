...
export function OfferCardRow({ offer, createdAt, hidden }: Props) {
  ...
  const isBuyer = (() => {
    if (!address || offer.allowedTaker === 0n) return false;
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center gap-1 shrink-0 ml-2">
          {isSeller && offer.allowedTaker === 0n && (
          ...
          {offer.allowedTaker !== 0n && (
            <span className="text-[9px] font-bold text-brand border border-brand/30 px-1.5 py-0.5 rounded-full">
              {isBuyer || isSeller ? 'Your Private OTC' : 'Private OTC'}
            </span>
          )}
          {hidden && (
            <span className="text-[9px] font-bold text-slate-400 border border-slate-600/40 bg-slate-800/40 px-1.5 py-0.5 rounded-full">
              Hidden
            </span>
          )}
          {offer.status !== 1 && (
